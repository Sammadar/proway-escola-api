...
def apgar_matricula(id: int, db: Session = Depends(get_db)):
    matricula = db.query(MatriculaEntidade).filter(MatriculaEntidade.id == id).firts()
    if matricula:
        db.delete(matricula)
        db.commit()
        return
    raise HTTPException(status_code=404, detail=f"Matricula não encontrado com id: {id}")
